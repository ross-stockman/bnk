...
@Repository
public class CustomerDAO implements SimpleDAO<CustomerResource, CustomerRequestParams, String, String> {
	...
	@Override
	public Optional<CustomerResource> get(String key) {
		try {
			return Optional.of(jdbcTemplate.queryForObject(
					"select " + String.join(",", DEFAULT_COLUMNS) + " from customer where _key = ?", (rs, rowNum) -> {
						return new CustomerResource(rs.getLong("id"), rs.getString("name"), rs.getString("_key"),
	...
	public List<CustomerResource> getAll(CustomerRequestParams params) {
		...
		var query = "select " + (String.join(",", columns)) + " from customer" + sorts + pagination;
		LOGGER.debug(query);
		return jdbcTemplate.query(query, (rs, rowNum) -> {
			var obj = new CustomerResource();
	...
	private List<String> resolveColumns(CustomerRequestParams params) {
	...
	private String resolveSorts(CustomerRequestParams params) {
	...
	private String resolvePagination(CustomerRequestParams params) {
	...
	@Override
	public String create(CustomerResource obj) {
		var key = keyProvisioner.getKey();
		jdbcTemplate.update("insert into customer (_key, id, name) values (?,?,?)", key, obj.getId(), obj.getName());
		return key;
	}

	@Override
	public void put(String key, CustomerResource obj) {
		jdbcTemplate.update("update customer set id = ?, name = ? where _key = ?", obj.getId(), obj.getName(), key);
	}

	@Override
	public void put(String key, String version, CustomerResource obj) {
		jdbcTemplate.update("update customer set id = ?, name = ?, _version = ? where _key = ?", obj.getId(),
				obj.getName(), key, version);
	}

	@Override
	public void delete(String key) {
		jdbcTemplate.update("delete from customer where _key = ?", key);
	}
